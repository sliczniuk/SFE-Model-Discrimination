function xdot = modelSFE(x, p, mask, dt, N, epsi_mask, one_minus_epsi_mask)
    ...
    %% States
    % ENTHALPY_RHO is stored scaled by 1/1e4 to normalise the ODE magnitude
    % (~-150,000 kJ/m3 -> ~-15).  Recover the physical value here for all
    % property evaluations; the derivative returned for this state is divided
    % by 1e4 at the bottom of the function.
    ENTHALPY_SCALE = 1e4;
    FLUID        = x(1:nstages_index);
    SOLID        = x(nstages_index+1:2*nstages_index);
    ENTHALPY_RHO = x(2*nstages_index+1:3*nstages_index) * ENTHALPY_SCALE;
    ...
    xdot = [
        ...
        % Enthalpy (derivative divided by ENTHALPY_SCALE because state is stored scaled)
        (-dHdz ./ one_minus_epsi_mask + dPdt + KRHOCP .* d2Tdz2) / ENTHALPY_SCALE;
