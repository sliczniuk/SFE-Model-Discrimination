function f_out = RBF_Function_3D_Single_Layer(x,y,z,N,parameters)
    ...
    %% Parameters for N RBFs
    cx = parameters(1 + (0*N :1*N-1)); % Centers of the RBFs in x
    cy = parameters(1 + (1*N :2*N-1)); % Centers of the RBFs in y
    cz = parameters(1 + (2*N :3*N-1)); % Centers of the RBFs in z
    w  = parameters(1 + (3*N :4*N-1)); % Weights of the RBFs
    sx = parameters(1 + (4*N :5*N-1)); % Widths of the RBFs in x (standard deviations)
    sy = parameters(1 + (5*N :6*N-1)); % Widths of the RBFs in y (standard deviations)
    sz = parameters(1 + (6*N :7*N-1)); % Widths of the RBFs in z (standard deviations)
    %w  = parameters(44 + (6*N :7*N-1)); % Weights of the RBFs
    b  = parameters(1 + (7*N+0     )); % Bias term
