function summary = run_single_start(run_idx, k0, which_k, Parameters, LabResults, bed_mask, timeStep_in_sec, ...
...
if run_idx == 1
    ...
    % Log-normal perturbation to stay positive; seed each run deterministically
    rng(100 + run_idx);
    perturb = exp(0.5 * randn(Nk, 1));
    k_init = max(1e-4, k0 .* perturb);
end
...
try
    ...
    K_out = opti.debug.value(k);
    obj = opti.debug.value(J);
    status = opti.stats();
end

summary.run = run_idx;
summary.k_init = k_init;
summary.K_out = K_out;
summary.obj = obj;
